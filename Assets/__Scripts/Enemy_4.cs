...
public class Part {
	...
	public GameObject go; //The GameoObject of this part
...
public class Enemy_4 : Enemy {
	...
	public float timeStart; //Birth tine for this Enemy_4
	...
	void OnCollisionEnter(Collision coll) {
		...
		case "ProjectileHero":
			...
			if(prtHit.protectedBy != null) {
				foreach( string s in prtHit.protectedBy ) {
					...
					if(!Destroyed(s)) {
						...
						return; //return because causing damage
					}
				}
			}
			...
			foreach(Part prt in parts) {
				if(!Destroyed(prt)) { //...change allDestroyed to false
					allDestroyed = false; //and break out of the foreach loop
					break;
				}
			}
